...
def retrieve_data(question_num):
    ...
    category, difficulty, question, answer = get_data.get_trivia_data(
        question_num)
    choices = get_data.get_choices()
...
def setup():
    global player1, player2, p1_answer, p2_answer, lock1, lock2
    ...
    for i in range(2):
        ...
        instruction_rect = ((width // 2), height - (height- 930))
        ...
        while True:
            ...
            timer_rect = instruction_text.get_rect()
            timer_rect = ((width // 2 + 500), height - (height- 930))
            seconds_in_total = seconds
            if 1 < seconds < 11:
                piece_of_the_bar = ((width / ((9)) / bar.smoothness_of_bar))
